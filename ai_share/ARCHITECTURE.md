...
- `Game` computes camera-to-NPC-root distance once per frame (the camera is unparented and NPC roots are scene children, as for mounts), then forwards it through `NPC.update(..., cameraDistance = 0)` and `CharacterCombatAnimator.update(dt, cameraDistance = 0)` to `MixerController.update(dt, cameraDistance = 0)`. Only visual evaluation uses the existing strict `>28m` / 12 Hz policy; AI, movement, combat events and gameplay timers still run every frame. All three mixers and their equipment pose layers remain together.
