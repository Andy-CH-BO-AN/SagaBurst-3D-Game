...
| Army Command `Q` | 回到上一層命令選單 |
| 列陣模式：`E` 或滑鼠左鍵 | 確認中央準星指定的列陣位置 |
| 列陣模式：`Q` | 回到命令選單 |
| `Esc` | 關閉 UI / 解除滑鼠鎖定 |

## 📯 Army Command 軍隊命令

Army Command 是一套**以鍵盤為主**的友軍指揮系統。HUD 會顯示在羅盤兩側，列出各兵種目前的命令狀態；所有友軍預設命令都是 **攻擊 Attack**。列陣位置可使用 `E` 或滑鼠左鍵確認。
...
2. 遊戲會即時顯示陣形 Preview。
3. 按 `E` 或滑鼠左鍵確認。
4. 按 `Q` 回到命令選單。列陣模式刻意不使用 `Esc`，避免解除 pointer lock。
