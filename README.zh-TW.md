...
- 每一關都允許選擇 T1 守軍，玩家可以自行用低 Tier 配置挑戰關卡。

| 關卡 | 守軍總上限 | 守軍 Tier 規則 | 攻軍 |
| --- | ---: | --- | --- |
...
| `E` | 拾取裝備 / 騎馬 / 下馬 |
| `Tab` 或 `I` | 開啟角色與背包介面 |
| 滑鼠滾輪 | 預設循環切換持有的近戰武器；命令模式下選擇兵種或命令 |
| `Q` | 切換滾輪命令模式；在命令子選單中優先返回上一頁 |
...
Army Command 的數字鍵快捷鍵隨時可用。按 `Q` 後，滾輪切換為選擇命令；在兵種頁再按一次 `Q`，滾輪回到切換持有的近戰武器。遠程武器仍透過右鍵瞄準使用。HUD 只顯示**實際有進場的友軍兵種**與全軍；第一個進場兵種預設凸起，命令模式下從第一個兵種往上滾即可選到**全軍 ALL**，再用滾輪上下移動其他可見兵種，中鍵進入目前凸起目標的命令。進入命令後預設凸起 **攻擊 Attack**，命令模式的滾輪可在攻擊 / 衝鋒 / 防禦 / 列陣之間移動，中鍵確認。在命令子選單或列陣位置按 `Q` 會返回上一頁，不會切換滾輪模式。列陣位置可使用 `E`、滑鼠左鍵或滑鼠中鍵確認。
...
- 在一般戰鬥入口加上 `mount=black-cat`，玩家起始坐騎即為黑貓；NPC 戰馬維持原設定。

造型依提供的黑貓三視圖製作，以肩高約 1.6m、頭至臀部長約 2.4m 為基準，包含黑毛、金色豎瞳、鬍鬚、長尾、貼背金邊皮鞍、縫線皮甲、白胸腹與白襪配色。這是原創程序化 3D 模型，並非從圖片重建的掃描資產；毛髮與解剖細節仍是遊戲化近似。

展示頁可切換正／側／背面；`1–5` 切換待機與步態、`6–9` 查看跳躍／落地／受擊／倒地、`Space` 暫停、`R` 重播、`V` 顯示騎士。試騎沿用 `WASD`、`Shift`、`Space` 與 `E` 上下騎。既有 `BLACK_CAT` 存檔會使用新外觀。
