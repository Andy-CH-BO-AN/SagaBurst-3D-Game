...
目前正式遊戲入口為 **Custom Battle 自訂戰役**：玩家可以配置 Viking 與 Roman 雙方軍隊、兵種、Tier、戰鬥部署模式、Player Faction，以及 Player 的起始裝備與騎乘狀態；兩邊 AI 軍隊則會在大型戰場上自動接戰。進入戰場後，玩家也可以透過鍵盤 **Army Command 軍隊命令**，對指定兵種或全軍下達攻擊、防禦、衝鋒與列陣命令。
...
| `Tab` 或 `I` | 開啟角色與背包介面 |
| `0` | 開啟遊戲選單 |
| Viking：`1`–`7` / Roman：`1`–`8` | 選擇友軍兵種或全軍，開啟 Army Command 命令選單 |
| Army Command `1` / `2` / `3` / `4` / `5` | 攻擊 / 防禦 / 衝鋒 / 列陣 / 退出 |
| 列陣模式：`Enter` 或 `NumpadEnter` | 確認中央準星指定的列陣位置 |
| 列陣模式：`5` | 取消列陣位置選擇 |
| `Esc` | 關閉 UI / 解除滑鼠鎖定 |

## 📯 Army Command 軍隊命令

Army Command 是一套**純鍵盤操作**的友軍指揮系統。HUD 會顯示在羅盤兩側，列出各兵種目前的命令狀態；所有友軍預設命令都是 **攻擊 Attack**。
...
| `2` | **防禦 Defend** | 原地防守。敵人已進入當前武器有效射程時可以轉向並攻擊，但不主動追擊；騎射手也不會繞圈。 |
| `3` | **衝鋒 Charge** | 主動追擊，體力允許時會 Sprint。Charge 本身不額外提供獨立傷害倍率。 |
| `4` | **列陣 Formation** | 使用畫面中央準星指定目的地，友軍移動到確定性的陣位；目前仍屬於該列陣命令的單位全部到位後，會自動切換為 **Defend**。 |
| `5` | **退出 Exit** | 關閉命令選單，不下達新命令。 |
...
2. 遊戲會即時顯示陣形 Preview。
3. 按 `Enter` 或 `NumpadEnter` 確認。
4. 按 `5` 取消。列陣模式刻意不使用 `Esc`，避免解除 pointer lock。
