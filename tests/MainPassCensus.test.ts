...
describe('on-demand main-pass attribution', () => {
  it('counts actual increments, excludes shadows and restores renderer instrumentation', () => {
    ...
    expect(result.passTotals.canvas.mainSubmissions).toBe(3)
    expect(result.passTotals.offscreen.mainSubmissions).toBe(0)
    expect(result.passTotals.totalNonShadow.triangles).toBe(24)
    expect(result.categoryPasses[0]).toMatchObject({ category: 'canvas/Terrain', mainSubmissions: 3 })
