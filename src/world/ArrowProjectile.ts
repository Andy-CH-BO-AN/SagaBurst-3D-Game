...
import { damageNpc, damagePlayer, type DamageResult } from '../combat/DamageRouter'
...
export class ArrowProjectile {
  ...
  update(
    ...
    if (this.shooterFaction === Faction.ENEMY && player.targetable) {
      ...
      if (dist <= 0.9) {
        const result = onDamagePlayer
          ? onDamagePlayer(this.damage)
          : damagePlayer(player, this.damage, (player as any)._hpBar ?? { setFill: () => {} } as any, null)
