...
export function prepareSwordHandShape(scene: THREE.Object3D, frame: SwordGripFrame): void {
  const hand = scene.getObjectByName('hand_r') as THREE.Bone
  ...
  scene.traverse(object => {
    ...
    bent.dispose()
    const morph = new THREE.Float32BufferAttribute(offsets, 3); morph.name = 'swordHand'
    ...
    object.userData.swordHandDiagnostics = { digits: fingers.digits.length, thumbClearance: shape.clearance }
  })
}
